...
import { BookingsScheduleView } from "./BookingsScheduleView";
...
export function BookingsTable({ bookings, onMutated }: BookingsTableProps) {
  const [viewMode, setViewMode] = useState<"schedule" | "matrix" | "list">(
    "schedule",
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <div className={styles.viewModePills} role="group" aria-label="Foglalások megjelenítése">
          <button
            type="button"
            className={`${styles.viewModeBtn} ${
              viewMode === "schedule" ? styles.viewModeBtnActive : ""
            }`}
            onClick={() => setViewMode("schedule")}
          >
            <Grid size={14} aria-hidden />
            <span>{adminContent.database.bookings.viewMode.schedule}</span>
          </button>
          ...
          >
            ...
            <span>{adminContent.database.bookings.viewMode.matrix}</span>
          </button>
          ...
          >
            ...
            <span>{adminContent.database.bookings.viewMode.list}</span>
          </button>
        </div>
      </div>

      {/* SCHEDULE & MATRIX VIEW */}
      {viewMode === "schedule" || viewMode === "matrix" ? (
        ...
        <div className={styles.panel}>
          <div className={styles.tableToolbar}>
            ...
            <div className={styles.filterRow}>
              ...
              >
                <option value="all">
                  {adminContent.database.bookings.filters.courtAll}
                </option>
              ...
              >
                <option value="all">
                  {adminContent.database.bookings.filters.typeAll}
                </option>
                <option value="season_pass">
                  {adminContent.database.bookings.filters.typeSeasonPass}
                </option>
              ...
              >
                <option value="all">
                  {adminContent.database.bookings.filters.statusAll}
                </option>
                <option value="confirmed">
                  {adminContent.database.bookings.filters.statusConfirmed}
                </option>
                <option value="cancelled">
                  {adminContent.database.bookings.filters.statusCancelled}
                </option>
              </select>
              ...
              >
                <option value="all">
                  {adminContent.database.bookings.filters.timeAll}
                </option>
                <option value="today">
                  {adminContent.database.bookings.filters.timeToday}
                </option>
                <option value="upcoming">
                  {adminContent.database.bookings.filters.timeUpcoming}
                </option>
                <option value="past">
                  {adminContent.database.bookings.filters.timePast}
                </option>
              </select>
            </div>
          </div>
              ...
              <thead>
                <tr>
                  <th className={styles.th}>
                    {adminContent.database.bookings.columns.court}
                  </th>
                  <th className={styles.th}>
                    {adminContent.database.bookings.columns.time}
                  </th>
                  <th className={styles.th}>
                    {adminContent.database.bookings.columns.user}
                  </th>
                  <th className={styles.th}>
                    {adminContent.database.bookings.columns.type}
                  </th>
                  <th className={styles.th}>
                    {adminContent.database.bookings.columns.players}
                  </th>
                  <th className={styles.th}>
                    {adminContent.database.bookings.columns.price}
                  </th>
                  <th className={styles.th}>
                    {adminContent.database.bookings.columns.status}
                  </th>
                  <th className={styles.th}>
                    {adminContent.database.bookings.columns.actions}
                  </th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <td colSpan={8} className={styles.td}>
                      <div className={styles.emptyState}>
                        <p className={styles.emptyBody}>
                          {adminContent.database.bookings.empty}
                        </p>
                      </div>
                    </td>
                  </tr>
